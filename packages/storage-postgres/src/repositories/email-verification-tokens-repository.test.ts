...
describe('email verification tokens repository', () => {
  it('creates and consumes token once with atomic update', async () => {
    ...
    const repo = createEmailVerificationTokensRepository(client);
    if (!repo) {
      throw new Error('Email verification token repository is unavailable.');
    }
    ...
    expect(created).toMatchObject({ tokenHash: 'token-hash', normalizedEmail: 'user@example.com' });
