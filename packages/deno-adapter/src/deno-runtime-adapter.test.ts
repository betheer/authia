...
import { createDenoRuntimeAdapter } from './deno-runtime-adapter.js';

describe('createDenoRuntimeAdapter', () => {
  it('returns notHandled when method/path do not match any configured route', async () => {
    const adapter = createDenoRuntimeAdapter(createConfig());
  ...
  it('returns RUNTIME_MISCONFIGURED when only one trusted forwarded header is present', async () => {
    const adapter = createDenoRuntimeAdapter(createConfig({ trustedForwardedHeaders: ['x-forwarded-host', 'x-forwarded-proto'] }));
  ...
  it('rejects credential-kind mismatch with entrypoint transport', async () => {
    const adapter = createDenoRuntimeAdapter(
      createConfig({
        entrypointTransport: {
  ...
  it('parses startOAuth body with provider and relative redirect', async () => {
    const adapter = createDenoRuntimeAdapter(createOAuthConfig());
  ...
  it('parses requestPasswordReset body with email', async () => {
    const adapter = createDenoRuntimeAdapter(createPasswordResetConfig());
