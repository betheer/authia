...
describe("createDefaultCryptoProvider", () => {
  it("hashes and verifies secrets", async () => {
    ...
    const valid = await provider.verifySecret(
  ...
  it("derives and verifies token verifiers", async () => {
    ...
    expect(await provider.verifyOpaqueToken("different-token", verifier)).toBe(
      false,
    );
  });
  ...
  describe("crypto hardening", () => {
    it("verifies opaque tokens correctly for matching and non-matching cases", async () => {
      ...
      const shortVerifier =
        typeof verifier === "string" ? verifier.slice(0, -2) : "";
      expect(await provider.verifyOpaqueToken(token, shortVerifier)).toBe(
        false,
      );

      // Verifier with same length but different content should fail
      const differentVerifier =
        typeof verifier === "string"
          ? verifier.slice(0, -1) +
            (verifier[verifier.length - 1] === "a" ? "b" : "a")
          : "";
      expect(await provider.verifyOpaqueToken(token, differentVerifier)).toBe(
        false,
      );
    });
    ...
    it("rejects verifier with null-byte padding (regression test)", async () => {
      ...
      expect(await provider.verifyOpaqueToken("token", "abc\u0000")).toBe(
        false,
      );
    });
    ...
    it("uses explicit Argon2id cost parameters when hashing", async () => {
      ...
      if (!isAuthError(result)) {
        ...
        const verifyResult = await provider.verifySecret(
          "test-password",
          result,
        );
  ...
  describe("error handling coverage", () => {
    it("maps hashSecret failure to CRYPTO_FAILURE", async () => {
      const provider = createDefaultCryptoProvider({
        hashSecret: async () => {
          throw new Error("hash error");
        },
      });
      const result = await provider.hashSecret("password");
      expect(result).toMatchObject({
        category: "infrastructure",
        code: "CRYPTO_FAILURE",
        retryable: false,
    ...
    it("maps verifySecret failure to CRYPTO_FAILURE", async () => {
      const provider = createDefaultCryptoProvider({
        verifySecret: async () => {
          throw new Error("verify error");
        },
      });
      const result = await provider.verifySecret("password", "hash");
      expect(result).toMatchObject({
        category: "infrastructure",
        code: "CRYPTO_FAILURE",
        retryable: false,
    ...
    it("maps deriveTokenId failure to CRYPTO_FAILURE", async () => {
      const provider = createDefaultCryptoProvider({
        deriveTokenId: async () => {
          throw new Error("derive id error");
        },
      });
      const result = await provider.deriveTokenId("token");
      expect(result).toMatchObject({
        category: "infrastructure",
        code: "CRYPTO_FAILURE",
        retryable: false,
    ...
    it("maps deriveTokenVerifier failure to CRYPTO_FAILURE", async () => {
      const provider = createDefaultCryptoProvider({
        deriveTokenVerifier: async () => {
          throw new Error("derive verifier error");
        },
      });
      const result = await provider.deriveTokenVerifier("token");
      expect(result).toMatchObject({
        category: "infrastructure",
        code: "CRYPTO_FAILURE",
        retryable: false,
    ...
    it("returns false if verifier is not a string in verifyOpaqueToken", async () => {
      const provider = createDefaultCryptoProvider();
      const result = await provider.verifyOpaqueToken("token", 123 as any);
      expect(result).toBe(false);
    });

    it("returns false if derived verifier is not a string in verifyOpaqueToken", async () => {
      const provider = createDefaultCryptoProvider({
        deriveTokenVerifier: async () => 123 as any,
      });
      const result = await provider.verifyOpaqueToken(
        "token",
        "string-verifier",
      );
      expect(result).toBe(false);
