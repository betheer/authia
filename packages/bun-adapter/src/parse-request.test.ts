...
describe('parseRequest', () => {
  ...
  describe('valid requests', () => {
    ...
    it('handles requests with no credentials if action allows it', async () => {
      const config = createConfig();
      // Assume sign in with password expects bearer, but none is provided. In this parseRequest logic,
      // expectedTransport MUST match. If credential is not provided but expectedTransport exists,
      // the logic actually checks `if (credential && credential.kind !== expectedTransport)`
      // Wait, let's look at parse-request.ts:
      ...
      // if (credential && credential.kind !== expectedTransport) { return invalidInputResult(); }
      // So if no credential is provided, it passes that check. And sets `transport: credential?.kind ?? expectedTransport`
      const result = await parseRequest({
        method: 'POST',
        url: 'https://example.com/auth/signin',
      ...
      expect(result).toEqual({
        ...
        method: 'POST',
        url: 'https://example.com/auth/signin',
        transport: 'bearer', // defaults to expectedTransport
        headers: {},
        cookies: {},
